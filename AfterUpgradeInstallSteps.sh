opkg update
opkg install ca-bundle openssl-util ca-certificates wget luci luci-ssl luci-compat luci-app-sqm luci-lib-ipkg openssh-sftp-server
...
wget $LUCI_APP_ARGON_CONFIG_LINK -O $LUCI_APP_ARGON_CONFIG_FILENAME
...
wget $LUCI_ARGON_THEME_LINK -O $LUCI_ARGON_THEME_FILENAME
opkg install $LUCI_ARGON_THEME_FILENAME

rm $LUCI_APP_ARGON_CONFIG_FILENAME $LUCI_ARGON_THEME_FILENAME
